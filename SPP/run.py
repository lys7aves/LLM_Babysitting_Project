...
SLEEP_RATE = 20 # sleep between calls

# def serialize_chat_completion(chat_completion):
#     # Extract necessary information from ChatCompletion object and return as a dictionary
#     return {
#         "completion_tokens": chat_completion.usage.completion_tokens,
#         "prompt_tokens": chat_completion.usage.prompt_tokens,
#         "choices": [choice.message.content for choice in chat_completion.choices]
...
def output_log_jsonl(log_file, all_logs):
    with open(log_file, "w") as f:
        ...
        for log_output in all_logs:
            # try:
            #     log_output = serialize_chat_completion(log_output)
            #     print(f"log_output: {log_output}============================")
            #     print(f"====================Serialized ChatCompletion object successfully to {log_output }")
            # except:
            #     print(f"======= not serialized with log_output: {log_output}============================")
            #     pass
...
def custom_serializer(response):
    if isinstance(response, openai.types.chat.chat_completion.ChatCompletion):
        # Accessing properties of the ChatCompletion object
        chat_id = response.id
        choices = response.choices
        created_at = response.created

        # Processing choices (which is likely a list of Choice objects)
        for choice in choices:
            # Access properties of Choice object
            finish_reason = choice.finish_reason
            index = choice.index
            message = choice.message  # This might be another object with its own properties

            # If message is a ChatCompletionMessage object, you can access its content
            message_content = message.content if message else None

            # Now you can process these as needed
            print(f"Choice {index}: {message_content}")
...
def _run_task(task_name, gpt, task, i, method, num_generation):
    if task_name in ['trivia_creative_writing', 'logic_grid_puzzle']:
        ...
        response = raw_response_batch[0]['response']
        print()
        print(f"============================  response type : { type(response) }")
        print()

        raw_response_batch[0]['response'] = custom_serializer(response)
